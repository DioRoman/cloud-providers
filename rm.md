...
### Проверка 2: Доступность приватных ВМ через NAT

1. Подключитесь к приватной ВМ через NAT-инстанс:
   ```bash
   # Получите IP NAT-инстанса
   NAT_IP=$(terraform output -raw vm_nat_ips | jq -r '.[0]')
   
   # Получите IP приватной ВМ
   PRIVATE_IP=$(terraform output -raw vm_private_ips | jq -r '.[0]')
   
   # Подключение через ProxyJump
   ssh -J ubuntu@$NAT_IP ubuntu@$PRIVATE_IP
   ```

2. Проверьте интернет-доступ:
   ```bash
   # Внутри приватной ВМ
   ping -c 4 8.8.8.8
   curl https://ya.ru
...
**Ожидаемый результат**: приватная ВМ имеет доступ в интернет через NAT-инстанс.
...
### Проблема: ВМ в группе не проходят health check

**Решение**:
1. Проверьте логи на инстансе:
   ```bash
...
3. Проверьте группы безопасности

### Проблема: Ошибка доступа к KMS-ключу
...
# Добавьте роль kms.keys.encrypterDecrypter
yc resource-manager folder add-access-binding <folder_id> \
  --role kms.keys.encrypterDecrypter \
  --service-account-name terraform-sa
```

### Проблема: Приватная ВМ не имеет доступа в интернет
...
2. Проверьте IP forwarding на NAT-инстансе:
   ```bash
   ssh ubuntu@<nat-ip>
